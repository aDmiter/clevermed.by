...
**Удалили врача/цену в админке, на сайте осталось:** после `npm run build` страницы `/doctors` и `/prices` были статическими. Сейчас они обновляются из БД при каждом заходе; после правок в админке достаточно `touch tmp/restart.txt` (без полного `npm run build`), если не меняли код.
